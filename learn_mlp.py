...
start_time = dt.now()
file = open("results_mlp.txt", mode='a')
...
# total_operations = len(df[u'operation'].unique()) + 1
...
model.add(Dense(total_operations))
...
file.write("-----------------------------------\n")
file.close()
save_model(model, "model_mlp.json", "model_mlp.weights.h5")
